...
| 二 · 粒子蛋糕 | 星光从四周汇聚成一个缓慢自转的三层粒子蛋糕，顶部一根金色蜡烛点亮 | 「闭上眼，许个愿」浮现，9 秒后蜡烛自动熄灭，全屏绽放烟花 |
| 三 · 气球与烟花 | 气球不断升起，画面中央提示「请为自己放个烟花吧」 | 按住画面 → 气球像蚁球一样朝按住处聚拢收紧；松手 → 落点附近气球一起炸开并补一簇全屏烟花。「二十岁 / 第一个以二开头的生日 / 山山巍然，祝你长青不晦」每 2 秒自动浮现一句，最后一句留在屏幕上 |
...
祝福文字的出现节奏在 [`src/styles/main.css`](src/styles/main.css) 的 `.wish-lines` 一段，改那几个 `2s` / `4s` 与 `@keyframes wishLineOnce` 即可。
